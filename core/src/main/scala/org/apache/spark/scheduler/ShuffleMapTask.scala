...
private[spark] class ShuffleMapTask(
  ...
  override def runTask(context: TaskContext): MapStatus = {
    ...
    // 所以task怎么拿到自已要处理的那个rdd的数据呢？？
    // 这里，会通过braoadcast variable, 直接拿到
    ...
    try {
      ...
      // 所以，核心的逻辑，就在rdd的iterator()方法中，在这里，就实现了针对rdd的某个partition，执行我们自已定义的
      // 算子，或者是函数
      // 执行完了我们自已定义的算子， 或者函数， 是不是相当于是针对rdd的partition的执行了处理，那么是不是会有返回
      //  的数据？？
      // ok, 返回的数据，都是通过ShuffleWriter，经过HashPartitioner进行分区之后，写入自已对应的分区bucket
      writer.write(rdd.iterator(partition, context).asInstanceOf[Iterator[_ <: Product2[Any, Any]]])
      // 最后，返回结果，MapStatus
      // MapStatus里边封装了ShuffleMapTask计算后的数据，存储在哪里，其实就是BlockManager相关的信息
