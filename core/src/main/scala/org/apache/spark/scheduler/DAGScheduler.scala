 ...
 * 输出是否被物化了（物化就是说，写入了磁盘或者内存等地方），并且寻找一个最少消耗（最优、最小）调度机制来运行job。
 * 它会将stage作为tasksets提交到底层的TaskSchedulerImpl上，来在集群上动行它们(task).
 * 
 * 除了处理stage的DAG， 它还会负责决定运行每个task的最佳位置，基于当前的缓存状态，将这些最佳位置提交给底层的
 * TaskSchedulerImpl。此外，它会处理由于shuffle输出文件丢失导致的失败，在这种情况下，旧的stage可能就会被重新
 * 提交。 一个satge内部的失败，如果不是由于shuffle文件丢失所导致的，会被TaskScheduler处理，它会多次重试每一个task，
...
class DAGScheduler(
  ...
  private def getShuffleMapStage(shuffleDep: ShuffleDependency[_, _, _], jobId: Int): Stage = {
    shuffleToMapStage.get(shuffleDep.shuffleId) match {
      ...
      case None =>
        ...
        val stage =
          newOrUsedStage(
            // Jed20190813：在此处shuffleDep.rdd应该就是MapPartitionsRDD、ShuffleRDD中的MapPartitionsRDD，
            // 代表其被划分新的父stage中去了，这里注意最终的那个父stage就是stag0
            shuffleDep.rdd, 
   ...
   * 但是，只要发现这个stage的rdd宽赖了某个rdd，那么就用宽依赖的那个rdd，创建一个新的stage
   * 然后立即将新的stage返回
   */
    ...
    def visit(rdd: RDD[_]) {
      if (!visited(rdd)) {
        ...
        if (getCacheLocs(rdd).contains(Nil)) {
          ...
          // 其实对于每一种有shuffle的操作，比如：groupByKey、reduceByKey、countByKey
          // 底层对应了三个RDD: MapPartitionsRDD、ShuffleRDD、MapPartitionsRDD
          for (dep <- rdd.dependencies) {
            dep match {
              // 如果是宽依赖
              case shufDep: ShuffleDependency[_, _, _] =>
                // 那么使用宽依赖的那个rdd, 创建一个stage, 并且会将isShulffleMap设置为true
                // 默认最后一个stage,不是shuffleMap stage
                // 但是finalStage之前所有的stage，都是shuffleMap stage
                // Jed20190813：在此处MapPartitionsRDD、ShuffleRDD中的MapPartitionsRDD被划分新的rdd中去了
                val mapStage = getShuffleMapStage(shufDep, stage.jobId)
                if (!mapStage.isAvailable) {
                  missing += mapStage
                  // 由于没有waitingForVisit加入新的rdd，所以此时要把这个missing中的那个父rdd返回出去
                }
  ...
  private[scheduler] def handleJobSubmitted(jobId: Int,
    ...
    try {
      ...
      // 创建一个Stage对象
      // 并且将stage加入DAGScheduler内部的内存缓 存中
    ...
    if (finalStage != null) {
      //  第二步，用finalSage，创建一个Job
      //  就是说，这个job的最后一个stage, 当然 就是我们的finalStage了
      ...
      if (shouldRunLocally) {
        ...
        // 并且导致其他所有的stage， 都 给放在watingStages队列里了
        submitStage(finalStage)
        
        // stage划分算法，实在太重要了，因为对于spark高手，或者spark精通人员来说
        // 必须对stage划分算法很清晰，直到你自已编写的spark application被划分为了几个job
        // 每个job被划分成了几个stage
        // 每个stage， 包括了你的哪些代码
        // 只有知道 了每个stage包括了你的哪些代码之后
  ...
  private def submitStage(stage: Stage) {
    ...
    if (jobId.isDefined) {
      ...
      if (!waitingStages(stage) && !runningStages(stage) && !failedStages(stage)) {
        // 调用getMissingParentStages()方法， 去获取当前这个stage父stage
   ...
   * 说白了，就是从stage的最后一个rdd开始，去找，哪一个rdd的partition， 是被cache了，或者checkpoint了
   * 那么，task的最佳位置，就是缓存的, checkpoint的partition位置
    ...
    : Seq[TaskLocation] =
  {
    ...
    // 最后，递归调用自已，去找寻找rdd的父rdd，看看对应的partition是否缓 存或者checkpoint
