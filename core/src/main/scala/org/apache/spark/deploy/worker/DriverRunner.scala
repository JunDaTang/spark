 ...
 */
/*
 * 管理一个driver的执行，包括在driver失败时自动重启driver,目前，这种方式仅仅使用于standalone集群部署模式
 */
    ...
    new Thread("DriverRunner for " + driverId) {
      override def run() {
        try {
          ...
          //第二步，下载用户上传的jar (我们编写完了spark应用程序，如果是java，用maven打个jar包如果是scala
          //,那么会用export将它导出为jar包)
